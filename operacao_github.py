...
# --- CONFIGURAÇÕES FIXAS ---
# Defina o ID aqui para execução automática (GitHub Actions)
...
class MasterFarmAnalyzer:
    ...
    # =========================================================================
    #  MÉTODOS AUXILIARES E GEOMETRIA
    # =========================================================================
    ...
    # =========================================================================
    #  MÉTODOS DE BUSCA E ESTRUTURA DE DADOS
    # =========================================================================
    def obter_canplugs_por_produtor(self, grower_id):
        # CORREÇÃO APLICADA: Tratamento robusto para 'installed_in' None
        ...
        for c in canplugs:
            ...
            if c.get('canplugID'):
                ...
                if iot: 
                    ...
                    # Verifica se installed_in existe E se é um dicionário válido
                    if installed_in and isinstance(installed_in, dict):
                        name = installed_in.get('name', 'Maq')
                    else:
                        name = f"Maq {c.get('canplugID')}"
    ...
    def _buscar_dados_climaticos_para_produtor(self, grower_id: int, start_date: str, end_date: str) -> pd.DataFrame:
        ...
        if estacoes_produtor.empty:
            print(f"AVISO: Nenhuma estação climática configurada.")
            return pd.DataFrame()
        ...
        for _, station in estacoes_produtor.iterrows():
            ...
            if raw_data:
                df_station = self._processar_clima_para_dataframe(raw_data, station_id, station['name'])
    ...
    def _merge_telemetria_clima(self, df_telemetria: pd.DataFrame, df_clima: pd.DataFrame, estacoes_produtor: pd.DataFrame) -> pd.DataFrame:
        if df_clima.empty or df_telemetria.empty:
            for c in ['temp_c', 'delta_t', 'vento_kph']: df_telemetria[c] = np.nan
            df_telemetria['CondicaoAplicacao'] = 'Sem Dados'
            return df_telemetria
        
        if len(estacoes_produtor) == 1:
            df_telemetria['nearest_station_id'] = float(estacoes_produtor.iloc[0]['id_estacao'])
        else:
            df_telemetria['nearest_station_id'] = df_telemetria.apply(
                lambda row: self._encontrar_estacao_mais_proxima(row['Latitude'], row['Longitude'], estacoes_produtor), axis=1
            ).astype('float64')
        ...
        def get_condicao(row):
            vento, dt = row['vento_kph'], row['delta_t']
            if pd.isna(dt) or pd.isna(vento): return 'Sem Dados'
            if dt >= 9 or vento > 10 or vento <= 1: return 'Evitar'
            if (2 <= dt <= 8) and (2 < vento <= 8): return 'Ideal'
            return 'Atenção'
            
        df_merged['CondicaoAplicacao'] = df_merged.apply(get_condicao, axis=1)
        return df_merged.drop(columns=['merge_date', 'merge_hour', 'nearest_station_id', 'station_id'], errors='ignore')
    ...
    def analisar_json_telemetria(self, json_data: dict | list, implement_id: int) -> pd.DataFrame:
        ...
        for feature in features_list:
            if feature.get('type') == 'Feature' and feature.get('geometry'):
                ...
                for key in list(props.keys()):
                    if isinstance(props[key], dict): props[key] = props[key].get('value', props[key].get('status'))
                ...
                props['Fuel Rate (L/h)'] = self._get_first_valid_value(props, ['Fuel Rate (L/h)', 'Fuel Consumption (L/h)'])
                
                width_m = self._get_first_valid_value(props, ['machine width (meters)', 'Implement Width (meters)'])
                if width_m is None and 'Header width in use (mm)' in props: width_m = props.get('Header width in use (mm)', 0) / 1000
                props['machine width (meters)'] = width_m
                
                props['Mass flow value'] = self._get_first_valid_value(props, ['Mass flow value', 'Mass flow (kg/sec)'])
        ...
        for col in ['velocity', 'Fuel Rate (L/h)', 'Engine RPM', 'machine width (meters)', 'Mass flow value', 'Grain Moisture (%)', 'Actual Engine - Percent Torque (%)', 'Coolant Temp (C)']:
            if col in df.columns: df[col] = pd.to_numeric(df[col], errors='coerce')

        try:
            area_por_hora_ha = (df['velocity'] * 1000 * df['machine width (meters)']) / 10000
            kg_por_hora = df['Mass flow value'] * 3600
            df['Produtividade (sc/ha)'] = np.where(area_por_hora_ha > 0, (kg_por_hora / area_por_hora_ha) / FATOR_KG_POR_SACO_PADRAO, np.nan)
            df.loc[df['Produtividade (sc/ha)'] > MAX_RENDIMENTO_REALISTA_SC_POR_HA, 'Produtividade (sc/ha)'] = np.nan
        except: df['Produtividade (sc/ha)'] = np.nan
    ...
    def _buscar_dados_com_chunking(self, implement_id: int, start_date_str: str, end_date_str: str) -> pd.DataFrame:
        ...
        print(f"   🚜 Buscando Telemetria: ID {implement_id}...")

        while current_date <= end_date:
            date_str = current_date.strftime('%Y-%m-%d')
            sys.stdout.write(f"      Dia {date_str}: ")
            for h in range(24):
                api_start, api_end = f"{date_str}T{h:02d}:00:00", f"{date_str}T{h:02d}:59:59"
                sucesso = False
                tentativas = 0
    ...
    def _identificar_blocos_operacao(self, df):
        ...
        df_trabalho['DateOnly'] = pd.to_datetime(df['Date']).dt.date
        micro_blocos = []
        ...
        for index, row in df_blocos.iterrows():
            if last_row is None:
                ...
                if is_same and date_diff == 1 and speed_change < VELOCIDADE_VARIACAO_LIMITE_PERCENT: pass
                else:
                    current_block_id = op_counter
                    op_counter += 1
    ...
    def executar_relatorio(self):
        ...
        if GROWER_ID_FIXO:
            ...
            implement_ids = todos_implementos
            largura_manual = None
        else:
            try:
                ...
                largura_manual = input("Largura manual (opcional): ").strip()
                largura_manual = float(largura_manual) if largura_manual else None
            except: print("Entrada inválida."); return
        ...
        # CORREÇÃO APLICADA: Cálculo de duração antes de gerar HTML
        print("⏱️ Calculando durações de pulso...")
        df_final = df_final.sort_values(['ImplementID', 'Timestamp (sec)'])
        df_final['duration_sec'] = df_final.groupby('ImplementID')['Timestamp (sec)'].diff().shift(-1)
        median_dur = df_final.loc[df_final['duration_sec'] <= MAX_PULSE_GAP_SECONDS, 'duration_sec'].median()
        if pd.isna(median_dur): median_dur = 10.0
        df_final['duration_sec'] = df_final['duration_sec'].fillna(median_dur)
        df_final.loc[df_final['duration_sec'] > MAX_PULSE_GAP_SECONDS, 'duration_sec'] = median_dur
    ...
    def gerar_html_master(self, df, data_inicio, data_fim):
        ...
        print("✅ Relatório 'index.html' gerado com sucesso!")
    ...
    def _get_html_template(self, eventos, data_inicio, data_fim):
        ...
        try:
            ...
            current = pd.to_datetime(data_inicio).date()
...
<body>
    ...
    <script>
        ...
        const GRADIENT_SCALE = ['#440154', '#2a788e', '#7ad151', '#fde725'];
        ...
        function redrawMap() {{
            ...
            if(!['CondicaoAplicacao'].includes(metric)) {{
                segments.forEach(s => {{
